...
/**
 * This class represents a dot shown in a FigureData visualization.
 * Thus it has coordinates and 'value' to be shown. Often this value
 * is shown by colorizing the dot using the FigureData's color range, however,
 * the value can also be shown as text.
 * 
 * The dot can also be a composite of a value and a prediction. In such cases
 * there is a larger dot below the value dot to allow visual comparison.
 * 
 * The dot size can be customized (and can dynamically change as a function of
...
public class GridDot {
    ...
    public double outerDotRadAdd = 0.4;
    ...
    public void qualityDotSizeMod(Float imprecisionRating, String ID) {
        if (!this.dottable()) return;
        if (imprecisionRating==null) return;
        qualityDotSizeMod(imprecisionRating); 
    }
